...
def expand_per_setting(d, model_settings, return_indices=False):
...
class ParameterWaveSetting:
    ...
    def read_PT_params(self, cube=None):
        ...
        if (self.PT_mode == 'Molliere') and (cube is not None):
            ...
            self.params['T_1'] = T_0 * (high - (high-low)*cube_copy[idx])
            cube[idx] = self.params['T_1']

            low, high = self.param_priors['T_2']
            idx = np.argwhere(self.param_keys=='T_2').flatten()[0]
            self.params['T_2'] = self.params['T_1'] * (high - (high-low)*cube_copy[idx])
    ...
    def read_uncertainty_params(self):
        ...
        for key_i in ['beta', 'a', 'l', 'a_f', 'l_f']:
            ...
            for j in range(self.n_orders):
                ...
                self.params[key_i][i,:] = self.params[f'{key_i}_{j+1}']
...
class Parameters:
    ...
    def __init__(
        ...
        free_params, self.n_params, self.unique_param_keys = expand_per_setting(
            free_params, self.model_settings_names, return_indices=True
            )
        constant_params = expand_per_setting(
            constant_params, self.model_settings_names
            )
        kwargs = {
            key_i: expand_per_setting(val_i, self.model_settings_names) \
