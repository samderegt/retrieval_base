...
sed -i 's/import config_DENIS as conf/import config_DENIS_MS09 as conf/g' retrieval.py
python retrieval.py --pre_processing

# Run the retrieval and evaluation
mpiexec -np $SLURM_NTASKS -use-hwthread-cpus --bind-to none python retrieval.py --retrieval
python retrieval.py --evaluation

# Revert to original config file
sed -i 's/import config_DENIS_MS09 as conf/import config_DENIS as conf/g' retrieval.py
